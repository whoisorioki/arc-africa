#!/usr/bin/env python3
"""
Corrected test script to debug neural guide loading and inference with proper tensor shapes.
"""
...
def test_model_inference_fixed(model):
    """Test basic model inference with correct tensor shapes."""
    print("\nTesting model inference with correct shapes...")
    try:
        model.eval()
        
        # Create dummy input with CORRECT shapes
        batch_size = 1
        num_pairs = 2  # input and output grids
        grid_size = 48
        max_colors = 10
        
        # Correct shape: (batch_size, num_pairs, grid_size, grid_size)
...
def test_loaded_model_inference(loaded_model):
    ...
    try:
        ...
        with torch.no_grad():
            predictions = loaded_model(input_grid, output_grid)
...
def main():
    """Main test function."""
    print("=== Neural Guide Debug Test (Fixed) ===\n")
    ...
    # Test model inference with correct shapes
    inference_success = test_model_inference_fixed(model)
    ...
    if loaded_model is not None:
        loaded_inference_success = test_loaded_model_inference(loaded_model)
    else:
        loaded_inference_success = False
    ...
    if not loaded_inference_success:
        print("4. Debug loaded model inference")
    print("5. Debug TTT adaptation process")
