"""
Simplified Context-Free Grammar (CFG) for the ARC Domain-Specific Language.

This is a simplified version that should work with our test programs.
"""

ARC_DSL_GRAMMAR_SIMPLE = r"""
    ?program: statement_list

    ?statement_list: statement
                   | statement_list "\n" statement -> statement_list_recursive

    ?statement: function_call
              | conditional_statement

    function_call: PRIMITIVE_NAME LPAR [arg_list] RPAR

    ?arg_list: expression ("," expression)*

    conditional_statement: "conditional_transform" LPAR condition "," function_call "," function_call RPAR
    
    ?condition: expression comparison_op expression

    comparison_op: GT | LT | EQEQ | NE | GE | LE

    ?expression: function_call
               | variable
               | literal

    variable: VARIABLE

    ?literal: ESCAPED_STRING
            | SIGNED_NUMBER
            | COLOR
            | "input"

    // Define explicit Terminals to remove all ambiguity
    PRIMITIVE_NAME: "colorfilter" | "fill" | "rotate90" | "horizontal_mirror" | "vertical_mirror" | "replace_color" | "compose" | "chain" | "find_objects" | "select_largest_object" | "select_smallest_object" | "count_objects" | "find_symmetry_axis" | "complete_symmetry" | "find_pattern_repetition" | "align_objects" | "crop" | "remove" | "move" | "segment_grid" | "get_color" | "conditional_transform"
    COLOR: "'red'" | "'blue'" | "'green'" | "'yellow'" | "'black'" | "'cyan'" | "'magenta'" | "'white'" | "'gray'"
    VARIABLE: /[a-zA-Z_][a-zA-Z0-9_]*/
    
    // Comparison operators
    GT: ">"
    LT: "<"
    EQEQ: "=="
    NE: "!="
    GE: ">="
    LE: "<="

    // Structural tokens
    LPAR: "("
    RPAR: ")"
    COMMA: ","

    %import common.ESCAPED_STRING
    %import common.SIGNED_NUMBER
    %import common.WS
    %ignore WS
"""

if __name__ == "__main__":
    print("--- Simplified ARC DSL Context-Free Grammar ---")
    print(ARC_DSL_GRAMMAR_SIMPLE)
    
    # Test the grammar
    from lark import Lark
    try:
        parser = Lark(ARC_DSL_GRAMMAR_SIMPLE, start='program')
        print("\n✅ Simplified grammar is syntactically valid and can be parsed by Lark.")
        
        # Test with our programs
        test_programs = [
            'colorfilter("blue")',
            'rotate90("cw")',
            'fill("red")',
            'chain(colorfilter("red"), rotate90("ccw"))',
            'conditional_transform(count_objects(input) > 2, colorfilter("blue"), rotate90("cw"))'
        ]
        
        for i, program in enumerate(test_programs, 1):
            try:
                tree = parser.parse(program)
                print(f"✅ Test {i}: '{program}' parsed successfully!")
            except Exception as e:
                print(f"❌ Test {i}: '{program}' failed - {e}")
        
    except Exception as e:
        print(f"\n❌ Error with simplified grammar: {e}") 